...
static void test_parse_array() {
    ...
    for (size_t i = 0; i < lept_get_array_size(&v); ++i) {
        const lept_value* tmp = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(tmp));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(tmp));
        for (size_t j = 0; j < lept_get_array_size(tmp); ++j) {
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(tmp, j)));
        }
    }
}
